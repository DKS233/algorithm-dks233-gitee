...
public class MedianSlidingWindow {
    public double[] medianSlidingWindow(int[] nums, int k) {
        ...
        for (int i = k - 1; i < nums.length; i++) {
            // 右边的数先进窗口
            sbTree.put(new Node(i, nums[i]));
            // k为奇数  7  0->1->2->3->4->5->6  中位数是7/2位置的数
            // k为偶数  8  0->1->2->3->4->5->6->7 中位数是8/2和(8-1)/2位置的数的平均值
            if (sbTree.getSize() % 2 == 0) {
                Node nodeOne = sbTree.getIndexKey(sbTree.getSize() / 2);
                Node nodeTwo = sbTree.getIndexKey((sbTree.getSize() - 1) / 2);
                ans[index++] = ((double) nodeOne.value + (double) nodeTwo.value) / 2;
            } else {
                ans[index++] = sbTree.getIndexKey(sbTree.getSize() / 2).value;
            }
            // 左边的数出窗口
